...
export default function UploadDatasetPage() {
  ...
  const cookie = getCookie("ouser");
  const userData = cookie ? JSON.parse(cookie as string) : null;
  const authorId = userData ? userData.id : null;
  ...
  //       try {
  //         const res = await api2.get(`ds/create/${datasetid}`);
  //         setDatasetDraft(res.data);
  //       } catch (err) {
  //         console.error(err);
  //         toast.error("Could not load dataset. Redirecting...");
  //         router.push("/ds/create");
  //       }
  //     };

  //     if (datasetid) fetchDataset();
  //   }, [datasetid, router]);
  ...
  //     const storedDatasets: Dataset[] = storedDatasetsRaw
  //       ? JSON.parse(storedDatasetsRaw)
  //       : [];
  ...
  const handlePublish = async () => {
    ...
    try {
      for (const file of files) {
        ...
        formData.append("commitMessage", "Initial upload"); // optional
        formData.append("file", file);

        await api2.post(`/datasets/${datasetid}/upload`, formData, {
          headers: {
      ...
      toast.success("Dataset published successfully!");
      router.push(`/u/${userData.username}`); // redirect to dataset page
    } catch (err: any) {
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to publish dataset");
    }
  };
    ...
    <div className="container mx-auto p-4 md:p-8">
      <header className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">
          Add Files to <span className="text-primary">Dataset</span>
        </h1>
        <p className="text-muted-foreground">
          Upload your data files and links. Click publish when you're ready.
        </p>
      </header>
