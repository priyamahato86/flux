  ...
  useEffect(() => {
    if (datasetid) {
      const fetchIssues = async () => {
        const { data } = await axios.get(`/api/issues?datasetId=${datasetid}`);
        console.log(data);
        setIssues(data.data);
      };
      fetchIssues();
    ...
    <div>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex-1">
          <Input placeholder="is:issue is:open" className="w-full md:w-auto" />
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button asChild className="bg-green-600 hover:bg-green-700">
            <Link href={`/ds/${datasetid}/issues/create`}>New issue</Link>
          </Button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-4 text-sm font-medium">
            <button className="flex items-center gap-1">
              <Dot className="text-green-500 h-6 w-6" /> Open{" "}
            ...
            {issues.map((issue) => (
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/ds/${datasetid}/issues/${issue.id}`}
                      className="font-semibold hover:text-blue-600"
                    >
                      {issue.title}
                    </Link>
                    {issue.labels.map((label) => (
                      <Badge key={label} className="border-transparent">
                        {label}
                      </Badge>
                    ))}
                  </div>
